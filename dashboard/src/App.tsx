...
import { StatsBar, LabelStatsPanel, RecentItemCard } from "./components/StatsAndRecent";
...
export function App() {
  ...
  return (
    <div class="app">
      ...
      <div>
        {recent.length === 0 && <div class="loading">No classifications yet</div>}
        {recent.map((item) => (
          <RecentItemCard key={item.pubkey} item={item} onSearch={searchProfile} />
        ))}
      </div>
    </div>
  );
